...
\begin{abstract}
排列流水線排程問題（Permutation Flowshop Scheduling Problem, PFSP）為一經典且具高度計算複雜性之組合最佳化問題。為最小化完工時間（Makespan），本研究提出一結合 NEH 建構式啟發法與動態禁忌（Dynamic Tabu）區域搜尋之迷因演算法（Memetic Algorithm, MA）。研究中首先探討不同交配算子（OX, LOX, PMX, CX）與突變策略對解空間結構保留之影響，證實線性順序交配（LOX）與插入突變（Insertion Mutation）能最有效維持遺傳優勢。其次，導入 NEH 演算法作為初始族群種子，顯著提升了演化起點與收斂速度。針對傳統靜態禁忌搜尋在演化後期容易引發「過度多樣化（Over-diversification）」進而削弱局部微調能力之缺陷，本研究創新引入基於適應度評估次數（FFE）的動態禁忌期限（Dynamic Tabu Tenure）機制。實驗結果顯示，透過退火式衰減的動態禁忌策略，演算法能完美平衡早期的全域探索（Exploration）與後期的局部開發（Exploitation），在 Taillard 基準測試集的中大型測資中皆獲得了極具競爭力的最佳解與高度穩定的平均表現。
\end{abstract}

\begin{IEEEkeywords}
Permutation Flowshop Scheduling Problem, Genetic Algorithm, Memetic Algorithm, NEH Heuristic, Dynamic Tabu Search
\end{IEEEkeywords}
...
\end{table}
表~\ref{tab:lox_tabu_comp} 呈現了 LOX 交配算子在三種 Tabu 設定下的實驗數據。在平均表現（Avg）方面，Dynamic Tabu 在中大型測資展現了顯著的修正效果。如 \texttt{tai50\_10\_1}，Dynamic Tabu 取得了最低的最佳解 3040，且平均完工時間 3105.75 亦優於其他版本。這說明隨 FFE 進度放寬限制的「退火」設計，能有效緩解靜態禁忌在後期過度抑制重複基因而阻礙局部開發的問題。
...
為了深入探討此現象，我們選取 \texttt{tai100\_20\_1} 觀察其收斂行為（如圖~\ref{fig:convergence_plots}）。圖中藍線為歷史最佳解，綠線為族群中位數。
...
\textbf{收斂曲線觀察分析：}
\begin{enumerate}
    \item \textbf{多樣性維持：} 在「Without Tabu」中，代表中位數的綠線在演化初期便迅速與藍線貼合，顯示族群快速同質化，導致後期搜尋幾乎陷入停滯。相對地，兩組 Tabu 實驗中，綠線與藍線始終保持明顯間距，證實了禁忌機制能有效防止「族群坍縮」。
    \item \textbf{階梯式突圍：} 觀察「Static Tabu」曲線，其藍線在演化中後期仍出現數次明顯的階梯式下降（如 600 代後）。這說明禁忌機制維持的多樣性，讓演算法在搜尋後期仍具備跳脫區域解並發現新全域最佳解的能力。
...
本研究成功設計並實作了一套針對排列流水線排程問題（PFSP）的高效迷因演算法（Memetic Algorithm）。透過嚴謹的模組化實驗與深度行為分析，我們得出以下幾項重要結論：
...
\begin{enumerate}
    ...
    \item \textbf{動態禁忌機制對探索與開發之完美平衡：} 本研究深入剖析了禁忌機制（Tabu Mechanism）在 GA 中的雙面刃效應。數據指出，固定的靜態禁忌長度雖能防止早熟收斂，卻在中大型測資中引發嚴重的「過度多樣化（Over-diversification）」，導致演算法在演化後期喪失針對優良盆地進行局部開發（Exploitation）的能力。為此，本研究提出的「基於 FFE 的動態禁忌期限（Dynamic Tabu Tenure）」成功解決了此一痛點。透過依據搜尋進度線性衰減禁忌約束的退火策略，演算法得以在早期的廣域探索與後期的深層微調之間取得完美平衡，最終在 \texttt{tai100\_20\_1} 等高難度實例中取得卓越的搜尋結果。
\end{enumerate}

未來的研究方向可考慮將此動態禁忌機制與其他先進的區域搜尋技術（如 Iterated Greedy 演算法）進行深度混種，或進一步引入自適應參數控制機制（Adaptive Parameter Control），以提升演算法在各類異質排程問題上的泛用性與強健性。
